...
class ExistingBasketTest < ApplicationSystemTestCase
  setup do
    @captured = []
    captured = @captured
  ...
  test "existing basket pause → user picks Replace → resume sent" do
    visit new_registration_path
    fill_in "user_email", with: "ebasket@example.com"
    ...
    post_callback(build.id, callback_base, "existing_basket_detected", { item_count: 3 })

    assert_text "You've got 3 items in your Tesco basket already", wait: 5

    click_on "Replace — start fresh"
    # click_on returns when the click is dispatched but not necessarily after the
    # POST/redirect/load round-trip has finished updating @captured. Wait briefly.
    Timeout.timeout(5) { sleep 0.05 until @captured.any? { |c| c.first == :resume } } rescue nil

    assert @captured.any? { |c| c.first == :resume && c.last == "replace" }
